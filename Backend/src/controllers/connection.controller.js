...
export const sendConnectionRequest = asyncHandler(async (req, res) => {
  ...
  if (!recipient.profileCompleted) {
    throw new AppError("Recipient profile is incomplete", 400);
  }
  ...
  if (existingConnection) {
    throw new AppError("Connection request already exists", 409);
  }
...
export const getIncomingRequests = asyncHandler(async (req, res) => {
  const requests = await Connection.find({
    ...
    .populate("requester", "name avatar city trustScore")
  ...
  const formattedRequests = requests.map((request) => ({
    ...
    requester: {
      ...
      trustScore: request.requester.trustScore,
    },
  }));
...
export const acceptConnectionRequest = asyncHandler(async (req, res) => {
  ...
  if (connection.status !== "pending") {
    throw new AppError(`Request already ${connection.status}`, 400);
  }
...
export const getConnections = asyncHandler(async (req, res) => {
  ...
  const connections = await Connection.find({
    ...
    .populate("recipient", "name avatar city trustScore")
  ...
  const formattedConnections = connections.map((connection) => {
    const otherUser =
      connection.requester._id.toString() === currentUserId.toString()
...
export const getSentRequests = asyncHandler(async (req, res) => {
  ...
  const formattedRequests = requests.map((request) => ({
    ...
    recipient: {
      ...
      trustScore: request.recipient.trustScore,
    },
  }));
